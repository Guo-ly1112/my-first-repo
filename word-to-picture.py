...
# 发起请求并返回结果
def main(appid,apikey,apisecret):
    input_text = '''生成一张图片：'''+sys.argv[1]
    print("Received text:", input_text)#命令行参数
    ...
    content = getBody(appid,input_text)
    print(time.time())
...
def parser_Message(message):
    ...
    if code != 0:
        ...
        #imageName = i
        savePath = f"C:\\Users\\67327\\Desktop\\final\\5-ywhy\\Assets\\Resources\\Images\\{imageName}.jpg"
        base64_to_image(imageBase,savePath)
        print("图片保存路径：" + savePath)
        #"D:\\游戏\\{imageName}.jpg"
        #"D:\游戏\UI练习\Assets\Resources\test.jpg"
...
if __name__ == '__main__':
    ...
    APPID = 'cdb50428'
    APISecret = 'NGNmNDE5MTY1MjhmMWQzMGIxMzQwZDRk'
    APIKEY = 'c6923052a5ac28ad0adf4660d571acae'
    #desc=input("请输入想要描述的图片内容：")
    
    #desc=get_input(text)
    # desc = '''生成一张图：四张小女孩的照片'''
    res = main(appid=APPID,apikey=APIKEY,apisecret=APISecret)
    # print(res)
    #保存到指定位置
    parser_Message(res)
